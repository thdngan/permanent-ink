import { Plugin, Editor, Notice, MarkdownView, EditorTransaction, WorkspaceLeaf, EditorChange, EditorPosition } from 'obsidian';
...
export default class StrikethroughOnKeyPlugin extends Plugin {
    ...
    private saveDataTimeout: number | null = null;
    ...
    async onload() {
        ...
        // We cast the workspace to `any` to bypass the strict TypeScript check.
        // This allows us to listen to the 'editor-change' event which, while not
        // in the public typings, is a stable and essential event.
        this.registerEvent(
            (this.app.workspace as any).on('editor-change', this.handleEditorChange)
        );
    }
    ...
    // We modify savePluginData to be "debounced" for performance.
    // We don't want to write to disk on every single keystroke.
    async savePluginData() {
        if (this.saveDataTimeout) {
            clearTimeout(this.saveDataTimeout);
        }
        this.saveDataTimeout = window.setTimeout(async () => {
    ...
    // --- NEW METHOD: THE CORE FIX ---
    /**
     * Handles editor changes to dynamically update annotation positions.
     */
    private handleEditorChange = (editor: Editor, change: EditorChange): void => {
        // Guard against changes we can't process.
        if (!change.to) {
            return;
        }
        
        // --- THE KEY FIX ---
        // Cast the incoming 'change' object to 'any'. This tells TypeScript to
        // trust us and allows us to access the real properties that exist at runtime,
        // like 'origin' and 'text' as a string array, without compilation errors.
        const detailedChange = change as any;

        // Now we can safely access the real properties.
        if (detailedChange.origin === 'setValue' || !detailedChange.origin) {
            return;
        }
        ...
        // The rest of this logic is now correct and will compile without issue,
        // because the compiler is no longer checking the properties on `detailedChange`.
        for (const annotation of annotations) {
            const isSelection = annotation.endLine !== undefined && annotation.endCh !== undefined;
            const from = { line: annotation.line, ch: annotation.ch };
            const to = isSelection ? { line: annotation.endLine!, ch: annotation.endCh! } : from;

            const changeStartsBeforeEnd = detailedChange.from.line < to.line || (detailedChange.from.line === to.line && detailedChange.from.ch < to.ch);
            const changeEndsAfterStart = detailedChange.to.line > from.line || (detailedChange.to.line === from.line && detailedChange.to.ch > from.ch);

            if (changeStartsBeforeEnd && changeEndsAfterStart) {
                annotationsToDelete.push(annotation.id);
                dataChanged = true;
                continue;
            }

            if (detailedChange.to.line < from.line || (detailedChange.to.line === from.line && detailedChange.to.ch <= from.ch)) {
                // Here, `detailedChange.text.length` works because `.text` is a string[] at runtime.
                const removedLines = detailedChange.to.line - detailedChange.from.line;
                const addedLines = detailedChange.text.length - 1; 
                const lineDelta = addedLines - removedLines;

                if (lineDelta !== 0) {
                    annotation.line += lineDelta;
                    if (isSelection) annotation.endLine! += lineDelta;
                }

                if (from.line === detailedChange.to.line) {
                    if (removedLines === addedLines) {
                        // `detailedChange.text[0]` works because .text is a string[] at runtime.
                        const charDelta = detailedChange.text[0].length - (detailedChange.to.ch - detailedChange.from.ch);
                        annotation.ch += charDelta;
                        if (isSelection && annotation.endLine === from.line) {
                            annotation.endCh! += charDelta;
                        }
                    } else {
                        const oldCharsOnLine = from.ch - detailedChange.from.ch;
                        const newCharsOnLine = detailedChange.text[detailedChange.text.length - 1].length;
                        const newCh = newCharsOnLine + oldCharsOnLine;
                        const charDelta = newCh - from.ch;
                        annotation.ch += charDelta;
                        if (isSelection && annotation.endLine === from.line) {
                            annotation.endCh! += charDelta;
                        }
        ...
        if (annotationsToDelete.length > 0) {
            this.data.annotations[file.path] = this.data.annotations[file.path].filter(
                a => !annotationsToDelete.includes(a.id)
            );
            new Notice(`Removed ${annotationsToDelete.length} annotation(s) because their text was modified.`);
        }
    ...
    onunload() {
        ...
        // It's good practice to clear any pending save timeouts when the plugin unloads
